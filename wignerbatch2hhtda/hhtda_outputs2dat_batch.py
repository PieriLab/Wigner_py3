#Written by Gregory Curtin, chatgpt was largely unhelpful for this so it doesnt get credit
#Two arguments: 1st is number of files to read from, 2nd is number of states you want to be printed
import re
import sys
...
column_names = ["Ex. Energy (eV)", "Osc. (a.u.)"]
...
file_to_print = './hhtda_batch_results.dat'
        ...
        for line in input:
            ...
            if extract_table:
                ...
                if len(parts) >= 7:
                    ...
                    energy.append(float(parts[4]))
                    oscillator_strength.append(float(parts[6])) 
...
transformed_file = 'transformed_hhtda_batch_results.dat'
...
formatted_file = 'formatted_hhtda_batch_results.dat'

def_max_column_pairs = 5  # Change this value to set the number of "s" and "o" pairs
...
with open(formatted_file, 'w') as outfile:
    ...
    for row_idx in range(num_rows):
        row = []
        for col_idx in range(0, num_columns * Total_files, Total_files * 2):  # Step by Total_files to process pairs of 's' and 'o'
